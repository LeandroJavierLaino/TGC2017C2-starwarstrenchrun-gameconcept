...
namespace TgcViewer.Utils.Gui
{
    ...
    public class gui_item
    {
        ...
        public int radio_button_group;      // indica que el item pertenece aun radio button group
    ...
    //static text
    public class gui_static : gui_item
    {
        public DrawTextFormat format;
        public gui_static(DXGui gui, String s, int x, int y, int dx = 0, int dy = 0 , 
                DrawTextFormat align= DrawTextFormat.Left | DrawTextFormat.VerticalCenter) :
        ...
        {
            gui.font.DrawText(gui.sprite, text, rc, format, DXGui.c_buttom_text);
        }
    }

    public class gui_line : gui_item
    {
        public gui_line(DXGui gui, int x, int y, int dx = 0, int dy = 0) :
        ...
        public override void Render(DXGui gui)
        {
            gui.DrawLine(rc.Left, rc.Top, rc.Right, rc.Top, 1, Color.WhiteSmoke);
        }
    }


    public class gui_imagen: gui_item
    {
        public gui_imagen(DXGui gui, string imagen,int x, int y, int dx = 0, int dy = 0) :
            base(gui, "", x, y, dx, dy, -1)
        {
            seleccionable = false;
            // Cargo la imagen en el gui
        ...
        public override void Render(DXGui gui)
        {
            if (textura != null)
            {
                gui.sprite.Draw(textura, Rectangle.Empty, Vector3.Empty, new Vector3(rc.Left,rc.Top,0),Color.White);
            }
        }
    }
    ...
    public class gui_button : gui_item
    {
        ...
        public override void Render(DXGui gui)
        {
            ...
            // recuadro del boton
            gui.DrawRect(rc.Left, rc.Top , rc.Right, rc.Bottom , 1,DXGui.c_buttom_frame);

            if (sel)
                // boton seleccionado: lleno el interior
                gui.DrawRect(rc.Left, rc.Top , rc.Right, rc.Bottom , 1,DXGui.c_buttom_selected, true);

            // Texto del boton
            Rectangle rc2 = new Rectangle(rc.Left, rc.Top + 2, rc.Width, rc.Height-2);
    ...
    // gui_radio_button
    public class gui_radio_button : gui_item
    {

        public gui_radio_button(DXGui gui, String s, int id, int x, int y, int dx = 0, int dy = 0) :
            base(gui, s, x, y, dx, dy, id)
        {
            seleccionable = true;
        }
        ...
        {
            bool sel = gui.sel == nro_item ? true : false;
            int radio = rc.Height-8;
            gui.DrawCircle(new Vector2(rc.Left + radio, rc.Top + rc.Height/2), radio, 3, Color.WhiteSmoke);

            if (marcado)
                gui.DrawCircle(new Vector2(rc.Left + radio, rc.Top + rc.Height / 2), 6, 6, Color.WhiteSmoke);

            // Texto del boton
            Rectangle rc2 = new Rectangle(rc.Left+radio+16, rc.Top, rc.Width, rc.Height);
            Color color = sel ? DXGui.c_buttom_sel_text : DXGui.c_buttom_text;
            gui.font.DrawText(gui.sprite, text, rc2, DrawTextFormat.VerticalCenter | DrawTextFormat.Left, color);
        }
    }

    // gui_check_button
    public class gui_check_button : gui_item
    {

        public gui_check_button(DXGui gui, String s, int id, int x, int y, int dx = 0, int dy = 0) :
            base(gui, s, x, y, dx, dy, id)
        {
            seleccionable = true;
        }
        ...
        public override void Render(DXGui gui)
        {
            bool sel = gui.sel == nro_item ? true : false;
            int radio = rc.Height - 8;
            gui.DrawRect(rc.Left , rc.Top+4, rc.Left+radio, rc.Bottom-4,3,Color.WhiteSmoke);

            if (marcado)
            {
                gui.DrawLine(rc.Left, rc.Top + 4, rc.Left + radio, rc.Bottom - 4, 3, Color.WhiteSmoke);
                gui.DrawLine(rc.Left + radio, rc.Top + 4, rc.Left, rc.Bottom - 4, 3, Color.WhiteSmoke);
            }
        ...
        public override void Render(DXGui gui)
        {
            ...
            switch (borde)
            {
                ...
                case frameBorder.redondeado:
                    // Interior
                    gui.RoundRect(rc.X, rc.Y, rc.X + rc.Width, rc.Y + rc.Height, 30, 6, c_fondo,true);
                    // Contorno
	        ...
	        // Texto del frame
            Rectangle rc2 = new Rectangle(rc.X, rc.Y, rc.Width, rc.Height);
		    rc2.Y += 20;
		    Color color = sel?c_selected:c_font;
		    gui.font.DrawText( gui.sprite, text, rc2, DrawTextFormat.NoClip | DrawTextFormat.Top | DrawTextFormat.Center, Color.FromArgb(gui.alpha,color));
        }
    }
    ...
    public class gui_tile_button : gui_item
    {
        ...
        public virtual void RenderGlyph()
        {
            ...
            if (textura != null)
            {
                ...
                gui.sprite.Transform = Matrix.Transformation2D(new Vector2(center.X * ex, center.Y * ey), 0, scale, new Vector2(0, 0), 0, offset);
